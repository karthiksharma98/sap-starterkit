...
def gen_results(db, out_dir, filename):
    ...
    for sid, seq in enumerate(tqdm(seqs)):
        ...
        results = pickle.load(open(join(out_dir, seq + '.pkl'), 'rb'))
...
def evaluate(db, out_dir, filename):
    ...
    def GetImageStream(self, request, context):
        ...
        for sid, seq in enumerate(self.seqs):
            ...
            while 1:
                ...
                fidx = int(np.floor(t_elapsed*self.opts.fps))
                if fidx == last_fidx:
                    continue
...
class EvalShell(cmd.Cmd):
    ...
    def __init__(self, img_server, img_servicer, log, db, out_dir, res_send):
    ...
    def do_gen_results(self, arg):
        ...
        if not arg.endswith('.json') and not arg.endswith('.pkl'):
            ...
            return
        gen_results(self.db, self.out_dir, arg)
        print("Wrote results to: " + arg)

    def do_evaluate(self, arg):
        'Evaluate and display result metrics.'
        evaluate(self.db, self.out_dir, arg)

    def do_log(self, arg):
        'View server log'
        os.system("cat " + self.log)
...
if __name__ == '__main__':
    ...
    with open(opts.log, 'w+', 1) as log:
        ...
        server.start()
        time.sleep(1)
        EvalShell(server, servicer, opts.log, db, opts.out_dir, res_send).cmdloop()
